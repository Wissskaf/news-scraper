...
async function lebanontwentyfour() {
  try {
    // Fetch data from the website
    const response = await fetch('https://www.lebanon24.com/live-news');
    if (!response.ok) {
    ...
    let existingData = [];
    if (fs.existsSync('lebanontwentyfour.json')) {
      const jsonData = fs.readFileSync('lebanontwentyfour.json', 'utf-8');
      existingData = JSON.parse(jsonData);
    }

    if (!arraysAreEqual(existingData.recentNews[105], articleData.recentNews[105])) {
      console.log('twentyfour has changed. Updating JSON file...');
      const jsonData = JSON.stringify(articleData, null, 2);
      fs.writeFileSync('lebanontwentyfour.json', jsonData);
      console.log('Data saved to lebanontwentyfour.json');
      // If data has changed, send a message to the Telegram bot
      bot.sendMessage(chatId, 'LB-NEWS\n\n' + articleData.recentNews[105] + '\n\n(Lebanon24)');
    } else {
      console.log('twentyfour has not changed.');
...
async function jadeed() {
  try {
    ...
    if (!arraysAreEqual(existingData.recentNews[95], articleData.recentNews[95])) {
...
const job0 = schedule.scheduleJob('*/1 * * * *', () => {
  console.log('Refreshing data...');
  lebanontwentyfour().catch(error => console.error(error));
});
